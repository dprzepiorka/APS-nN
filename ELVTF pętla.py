...
METHOD = "CEO"  # "PSO", "CEO", "PO", "DOA", "RTO", "SPO- długi", "BPB", "KLA", "KEO", "DOE", "OSA", "ECO", "GOA", "COO"
...
STORAGE_P_MIN = -50.0
STORAGE_P_MAX = 50.0
# Keep Q bounds for clipping final reactive power values
STORAGE_Q_MIN = -30.0
STORAGE_Q_MAX = 30.0
# Tangent of phi bounds (Q/P). E.g. ±3 corresponds to poor power factor extremes.
STORAGE_TANPHI_MIN = -0.75
STORAGE_TANPHI_MAX = 0.75
...
# Example tuned weights (can be changed)
WEIGHTS = np.array([0.126308, 0.800897, 0.028356, 0.028608, 0.016131], dtype=float)
...
def apply_solution_with_storage(app, vars_def, x):
    ...
    New behavior:
      - storage_P1..P3 are active powers [kW] per phase
      - storage_tan1..3 are tangents of phi (Q/P)
      - reactive per-phase Qk = Pk * tanphi_k
    """
    ...
    # read storage P and tanphi variables positions (expected order)
    # Expect names: storage_P1, storage_P2, storage_P3, storage_tan1, storage_tan2, storage_tan3
    try:
        ...
        tan1 = float(x[idx_p1 + 3]); tan2 = float(x[idx_p1 + 4]); tan3 = float(x[idx_p1 + 5])
        # compute reactive powers Q = P * tan(phi)
        Q1 = P1 * tan1
        Q2 = P2 * tan2
        Q3 = P3 * tan3
    except Exception:
        # if any var missing or wrong ordering, fallback to old behavior (try to find Q vars)
        try:
            # Try legacy names: storage_Q1 etc.
            idx_q1 = next(i for i, v in enumerate(vars_def) if v["name"] == "storage_Q1")
            # assume P vars are before Q vars in legacy layout
            P1 = float(x[idx_q1 - 3]); P2 = float(x[idx_q1 - 2]); P3 = float(x[idx_q1 - 1])
            Q1 = float(x[idx_q1]); Q2 = float(x[idx_q1 + 1]); Q3 = float(x[idx_q1 + 2])
        except Exception:
            # give up and do nothing (no storage applied)
            return candidate, None

    # Clip Q to safe bounds (using STORAGE_Q_MIN/MAX)
    Q1 = max(STORAGE_Q_MIN, min(STORAGE_Q_MAX, Q1))
    Q2 = max(STORAGE_Q_MIN, min(STORAGE_Q_MAX, Q2))
    Q3 = max(STORAGE_Q_MIN, min(STORAGE_Q_MAX, Q3))

    # Apply storage to node and return previous values to restore later
...
def main():
    ...
    if K > 0:
        # add storage selection index + per-phase P and per-phase tan(phi) variables
        vars_def.append({
        ...
        # per-phase active power (P1,P2,P3) and per-phase tangents of phi (tanphi1,tanphi2,tanphi3)
        vars_def += [
            ...
            {"name": "storage_tan1", "pf_class": "storage", "attr": "", "min": STORAGE_TANPHI_MIN, "max": STORAGE_TANPHI_MAX},
            {"name": "storage_tan2", "pf_class": "storage", "attr": "", "min": STORAGE_TANPHI_MIN, "max": STORAGE_TANPHI_MAX},
            {"name": "storage_tan3", "pf_class": "storage", "attr": "", "min": STORAGE_TANPHI_MIN, "max": STORAGE_TANPHI_MAX},
        ]
