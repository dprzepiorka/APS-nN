...
# ---- Remote stop control file ----
# When set, a file is created at this path at the start of optimization.
# To stop gracefully: delete the file OR write "0" / "stop" into it.
# Set to "" to disable this feature.
CONTROL_FILE = r"N:\ksiz\STUDIA_DOKTORANCKIE\PRZEPIORKA\Modele sieci\Niskie napiecie\APS\Optymalizacja\OPT_CONTROL.txt"

# ---- Early stopping ----
# Stop when best objective reaches this value (set to 0.0 to disable).
EARLY_STOP_THRESHOLD = 0.0
# Stop after this many iterations without any improvement.
EARLY_STOP_PATIENCE = 200

# ---- Progress reporting ----
# Print a progress summary to console every N function evaluations.
    ...
    try:
        ...
        try:
            ...
            if fraction_nonzero < FRACTION_NONZERO_THRESHOLD:
                ...
                COMPONENT_HISTORY.append(comps + [big_pen, big_pen])
                return big_pen
        except Exception as _sanity_ex:
            # sanity check itself threw an exception (e.g. GetCalcRelevantObjects unavailable)
            # -> treat as failed LDF, return large penalty (do NOT silently continue)
            big_pen = min(PENALTY * float(LARGE_PENALTY_MULTIPLIER), LARGE_PENALTY_CAP)
            with open("failed_evals.csv", "a", encoding="utf-8") as f:
                f.write(f"{time.time()},{','.join(map(str, x))},sanity_check_exception,{str(_sanity_ex)}\n")
            comps = [np.nan] * 5
            COMPONENT_HISTORY.append(comps + [big_pen, big_pen])
            return big_pen
        ...
        # Guard: if ALL components are exactly zero the LDF silently failed
        # (PowerFactory returned code 0 but produced no results). Return large penalty.
        if total_obj == 0.0 and penalty == 0.0 and all(c == 0.0 for c in [C1, C2, C3, C4, C5]):
            big_pen = min(PENALTY * float(LARGE_PENALTY_MULTIPLIER), LARGE_PENALTY_CAP)
            with open("failed_evals.csv", "a", encoding="utf-8") as f:
                f.write(f"{time.time()},{','.join(map(str, x))},all_zero_components_silent_ldf_fail,0\n")
            comps_nan = [np.nan] * 5
            COMPONENT_HISTORY.append(comps_nan + [big_pen, big_pen])
            return big_pen
...
# -------------------------
def create_control_file(path: Path) -> bool:
    """
    Create a control file with value "1".
    While "1" is in the file the optimization runs.
    Delete the file or write "0"/"stop" to halt gracefully.
    """
    try:
        path.write_text("1", encoding="utf-8")
        print(f"Plik kontrolny utworzony: {path}")
        print("  -> Aby zatrzymac optymalizacje: usun plik lub wpisz '0' / 'stop'.")
        return True
    except Exception as e:
        print(f"Ostrzezenie: nie mozna utworzyc pliku kontrolnego: {e}")
        return False


def check_stop_signal(path: Path) -> bool:
    """
    Return True when a stop has been requested (file missing or contains 0/stop).
    """
    try:
        if not path.exists():
            return True           # plik usuniety = stop
        content = path.read_text(encoding="utf-8").strip()
        return content in ("0", "stop", "STOP", "Stop")
    except Exception:
        return False              # blad odczytu = kontynuuj
...
def main():
    ...
    _ctrl = Path(CONTROL_FILE) if CONTROL_FILE else None
    if _ctrl:
        create_control_file(_ctrl)

    def obj(x):
        _tracker["eval_count"] += 1
        n = _tracker["eval_count"]

        # Sprawdz plik kontrolny co 10 ewaluacji
        if _ctrl and n % 10 == 0:
            if check_stop_signal(_ctrl):
                _tracker["stop_requested"] = True
        if _tracker["stop_requested"]:
            # Zwroc dotychczas najlepsza wartosc - optymalizator zatrzyma sie naturalnie
            return _tracker["best_val"] if _tracker["best_x"] is not None else float(PENALTY * LARGE_PENALTY_MULTIPLIER)

        val = objective_function(app, vars_def, x, ldf)

        # Czy to odrzucona ewaluacja (blad LF)
        if val >= PENALTY:
            _tracker["fail_count"] += 1

        elapsed = time.time() - (_tracker["time_start"] or time.time())

        if val < _tracker["best_val"]:
            _tracker["best_val"] = val
            _tracker["best_x"] = np.array(x, dtype=float)
            msg = (f"[ewal #{n} | {elapsed:.1f}s] NOWE MINIMUM: f = {val:.6f}  "
                   f"(odrzucono: {_tracker['fail_count']}/{n} = "
                   f"{100*_tracker['fail_count']/n:.1f}%)")
            print(msg)
            try:
                app.PrintPlain(msg)
            except Exception:
                pass
        elif n % PROGRESS_PRINT_EVERY == 0:
            pct_fail = 100 * _tracker["fail_count"] / n
            msg = (f"[ewal #{n} | {elapsed:.1f}s] Postep: best = {_tracker['best_val']:.6f}  "
                   f"odrzucenia: {_tracker['fail_count']}/{n} ({pct_fail:.1f}%)")
            print(msg)
            try:
                app.PrintPlain(msg)
            except Exception:
    ...
    _tracker["time_start"] = time_start
    # Choose optimizer
    res = None
    ...
    if METHOD_U == "PSO":
        pso = PSO(obj, N_PARTICLES, Dim, Lb, Ub, N_ITER, W, C1, C2,
                  ...
                  early_stop_threshold=EARLY_STOP_THRESHOLD, early_stop_patience=EARLY_STOP_PATIENCE)
        res = pso.optimize()
    elif METHOD_U == "CEO":
    ...
    _n = max(1, _tracker["eval_count"])
    _fail = _tracker["fail_count"]
    _accepted = _n - _fail
    print("\n" + "="*60)
    print("STATYSTYKI OPTYMALIZACJI")
    print("="*60)
    print(f"  Czas laczny:        {time_end - time_start:.2f} s")
    print(f"  Ewaluacji total:    {_n}")
    print(f"  Odrzucone (bl. LF): {_fail} ({100*_fail/_n:.1f}%)")
    print(f"  Akceptowane:        {_accepted} ({100*_accepted/_n:.1f}%)")
    print(f"  F_best:             {_tracker['best_val']:.6f}")
    if _tracker["stop_requested"]:
        print("  STATUS: ZATRZYMANO przez plik kontrolny")
    print("="*60 + "\n")

    # Usun plik kontrolny po zakonczeniu
    if _ctrl:
        try:
            _ctrl.unlink(missing_ok=True)
        except Exception:
            pass
